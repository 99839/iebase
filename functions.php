...
function iebase_setup() {
  ...
  */
  function iebase_archive_postcount_filter( $variable ) {
    $variable = str_replace( '(', '<span class="post-count"> ', $variable );
    $variable = str_replace( ')', ' </span>', $variable );
    return $variable;
  }
  add_filter( 'get_archives_link', 'iebase_archive_postcount_filter' );
  ...
  */
  ...
  function iebase_custom_comment_form_fields( $fields ) {
    ...
    $fields['author'] = '<div class="e-grid"><div class="e-grid__col e-grid__col--4-4-s e-grid__col--1-3-m">'.
      '<p><input id="author" name="author" type="text" placeholder="' . esc_html__( 'Name', 'iebase' ) . ( $req ? ' *' : '' ) . '" value="' . esc_attr( $commenter['comment_author'] ) . '" size="20"' . $aria_req . ' /></p></div>';

    $fields['email'] = '<div class="e-grid__col e-grid__col--4-4-s e-grid__col--1-3-m">'.
      '<p><input id="email" name="email" type="text" placeholder="' . esc_html__( 'Email', 'iebase' ) . ( $req ? ' *' : '' ) . '" value="' . esc_attr( $commenter['comment_author_email'] ) . '" size="20"' . $aria_req . ' /></p></div>';

    $fields['url'] = '<div class="e-grid__col e-grid__col--4-4-s e-grid__col--1-3-m">'.
      '<p><input id="url" name="url" type="text" placeholder="' . esc_html__( 'Website', 'iebase' ) . '" value="' . esc_attr( $commenter['comment_author_url'] ) . '" size="20"' . $aria_req . ' /></p></div></div>';
